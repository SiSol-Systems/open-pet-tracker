...
function anyscreen(css_urls, successCallback){
	...
	/* measure truly available sizes and set app.container*** */
	...
	console.log("app.containerWidth: " + app.containerWidth);
	console.log("app.containerHeight: " + app.containerHeight);

	// After adding viewport perform some checks

    var calculatedContainerWidth = app.deviceWidth * dpr;
    var calculatedContainerHeight = app.deviceHeight * dpr;
    
    /* measuredwidth/calculatedWidth = congruence in % as decimal number */
    var width_congruence = Math.min(app.containerWidth,calculatedContainerWidth) / Math.max(app.containerWidth, calculatedContainerWidth);
	var height_congruence = Math.min(app.containerHeight,calculatedContainerHeight) / Math.max(app.containerHeight, calculatedContainerHeight);

	var congruence = Math.max(width_congruence, height_congruence);

	/* adjust app.deviceHeight app.deviceWidth if necessary */
    if (calculatedContainerWidth == app.containerWidth || congruence > 0.9){
        app.deviceHeight = app.deviceHeight * dpr;
        app.deviceWidth = app.deviceWidth * dpr;
    }
    else {	
		app.deviceWidth = app.containerWidth;
		app.deviceHeight = app.containerHeight;
        console.log("error rescaling screen, falling back to measurements");
    }


	/* fix system input elements in size */
	var input_scale_factor = (app.containerWidth / screen.width);

	/*if (input_scale_factor < dpr){
		input_scale_factor = dpr;
	}*/

	var percentage_factor = parseInt((1/input_scale_factor) * 100);

	var input_styles = document.createElement("style");
	document.head.appendChild(input_styles);
	input_styles.sheet.insertRule('input[type="range"], input[type="radio"] { transform: scale(' + input_scale_factor + '); }');
	input_styles.sheet.insertRule('input[type="range"] { transform-origin: 0 50% 0; height:auto; width: ' + percentage_factor + '%; }');
	input_styles.sheet.insertRule('input[type="radio"] { transform-origin: 50% 50% 0; }');
	
	/* create handlebars helper if handlebars is present */
	if (app.deviceWidth <= 640){
		imgFolder = "img/640down/";
	}
	else if (app.deviceWidth <= 1080){
		imgFolder = "img/1080down/";
	}
	else {
		imgFolder = "img/1720down/";
	}

	if (typeof window.Handlebars != "undefined" && window.Handlebars != null){

		Handlebars.registerHelper("imageFolder", function () {
		    return imgFolder;
		});
	}
		...
		xhr.onreadystatechange = function () {
	        ...
	        if (xhr.readyState == 4) {
		        ...
		        if (xhr.status == 200 || xhr.status == 0 ) {
		   			...
		   			var adapted_css_str = adaptCSS(xhr.responseText);
                ...
                if(pol == "\n" || pol==";" || str[i] == "}"){
                    ...
                    var new_val = "";
	   				...
	   				for (var v=0; v<val_parts.length; v++){
	   					...
	   					if (val_part.indexOf("px") > 0 || val_part.indexOf("vw") > 0 || val_part.indexOf("vh") > 0){
	   						var number_value = parseFloat(val_part.substr(0, val_part.length - 2));

							if (number_value != 0){

								/* support vw, vh */
								if (val_part.indexOf("vw") > 0){
									/* polyfill for vw. 1vw = 1% of the viewport */
									var new_pixel_value_floored = Math.floor((number_value/100) * app.containerWidth);
								}
								else if (val_part.indexOf("vh") > 0){
									var new_pixel_value_floored = Math.floor((number_value/100) * app.containerHeight);
								}
								else {
