window.datePicker = {
	template_name : "themes/templates/datepicker.html",
	template : null,
	_minDate : new Date(0), //default
	minDate : null, //atomic
	_maxDate : new Date(), //default
	maxDate : null,
	_date : new Date(), // the date of this object
	date : null,
	selects : {},
	show : function(options, onsuccess, onerror){
		...
		self._load_template(function(){
			...
			var selects = ModalDialog.content.querySelectorAll("select");

			for (var s=0; s<selects.length; s++){
				var select = selects[s];
				self.selects[select.getAttribute("name")] = select;
				select.addEventListener("change", function(event){
					var name = event.target.getAttribute("name");

					self.date[name] = event.target.value;
					...
					for (var p=parts.indexOf(name)+1; p<parts.length; p++){
	...
	_reload_part : function(part_name){
		...
		// iterate over all months and check with select
